...
<div id="content">
	...
	<div class="well">
		<select name="plan" id="plan" onchange="location.href='notas.jsp?plan='+this.options[this.selectedIndex].value;" style="margin-bottom:0;">
		...
		for(int j=0; j<lisAlumnoCurso.size(); j++){
			...
			if(alumnoCurso.getCursoId().equals(curso.getCursoId())){
				...
				if (grado!=gradoTemp){
					...
					//System.out.println("CICLOID:"+alumnoCurso.getCicloId());
					...
					%>
						...
						<tr class="alert">
							<td colspan="3" align="right">Promedio General:&nbsp;&nbsp;&nbsp;&nbsp;</td>
							<%for(int l = 0; l < lisBloque.size(); l++){ 
								...
								if(sumaPorBimestre[l] > 0 && cantidadMaterias > 0){ 
			    					...
			    					sumaPorBimestre[l] = sumaPorBimestre[l]/(cantidadMateriasTmp+1);
								}
							%>
...
%>
					<div class="alert alert-info">
						<%=aca.catalogo.CatNivelEscuela.getNivelNombre(conElias, escuelaId, nivel) %> - <%=aca.catalogo.CatNivel.getGradoNombre(grado)%> <%= aca.catalogo.CatNivelEscuela.getTitulo(conElias, escuelaId, nivel).toUpperCase() %></b></td>
					</div>
...
%>
							<th width="5%"title="<%=cicloBloque.getBloqueNombre() %>"><%=k+1%></th>
<%					} %>
					    <th width="5%"><fmt:message key="aca.Nota" /></th>
					    <th width="5%"><fmt:message key="aca.FechaNota" /></th>
					    <th width="5%"><fmt:message key="aca.Extra" /></th>
					    <th width="5%"><fmt:message key="aca.FechaExtra" /></th>
					</tr>
<%				}	%>
