...
<%
	...
	if (accion.equals("1")) { //Guardar Faltas
		...
		for (aca.kardex.KrdxCursoAct kardex : lisKardexAlumnos) {
			...
			if (tardanza != null) {
				if (tardanza.equals("")){//Si no tiene nota entonces eliminala si es que existe, si no pues ignora esa nota
...
<div id="content">
	<h2><fmt:message key="maestros.RegistrodeTardanzas" /> <small><%=empPersonal.getNombre() + " " + empPersonal.getApaterno() + " " + empPersonal.getAmaterno()%></small></h2>
	...
	<form action="evaluarTardanzas.jsp?CursoId=<%=cursoId %>&CicloGrupoId=<%=cicloGrupoId %>" name="forma" method="post">
			...
			%>
				<tr>
					...
					<%
						...
						String total = "-";
						if(sumaTardanzas != 0){
							total = getformato.format(sumaTardanzas).replace(',','.');
						}
					%>	
