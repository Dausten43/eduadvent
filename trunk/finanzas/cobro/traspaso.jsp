...
<% 
	...
	//System.out.println("CICLO ID ORIGINAL : "+cicloIdOriginal);
	//System.out.println("PERIODO ID ORIGINAL : "+periodoIdOriginal);
	String cicloIdClon			= request.getParameter("cicloId")==null?"0":request.getParameter("cicloId");
	String periodoIdClon		= request.getParameter("periodoId")==null?"1":request.getParameter("periodoId");
	//System.out.println("CICLO CLON : "+cicloIdClon);
	//System.out.println("PERIODO CLON : "+periodoIdClon);
	...
	//System.out.println("CICLO ELEGIDO : "+cicloElegido);
	String mensaje				= "";
...
// 		}
	...
	ArrayList<aca.ciclo.CicloPeriodo> lisCicloPeriodoClonado = cicloPeriodoL.getListCiclo(conElias, cicloIdClon, "ORDER BY CICLO_PERIODO.F_INICIO");
	boolean encontro = false;
	for(aca.ciclo.CicloPeriodo per : lisCicloPeriodoClonado){
		if(per.getPeriodoId().equals(periodoIdClon)){
			encontro = true;
		}
	}
	if(encontro==false && lisCicloPeriodoClonado.size()>0){
		periodoIdClon = lisCicloPeriodoClonado.get(0).getPeriodoId();
	}
	...
	ArrayList<aca.fin.FinPago> lisFinPagoCopia = finPagoL.getListCicloPeriodo(conElias, cicloIdClon, periodoIdClon, "ORDER BY FIN_PAGO.FECHA, DESCRIPCION");
...
<%-- 					<select id="ciclo" name="ciclo" onchange="document.location = 'traspaso.jsp?ciclo='+this.options[this.selectedIndex].value+'&ciclo=<%=cicloIdOriginal%>&periodo=<%=periodoIdOriginal%>';" style="width:360px;margin-bottom:0px;"> --%>
					<select id="ciclo" name="ciclo" onchange="document.location = 'traspaso.jsp?ciclo='+this.options[this.selectedIndex].value" style="width:360px;margin-bottom:0px;">
				<%
