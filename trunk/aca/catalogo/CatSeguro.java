...
public class CatSeguro {
	...
	public boolean updateReg(Connection conn ) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("UPDATE CAT_SEGURO" +
					" SET POLIZA = ?, " +
					" WHERE ESCUELA_ID = TO_NUMBER(?, '999') AND  YEAR = TO_NUMBER(?, '999')");
	...
	public void mapeaRegId(Connection con, String escuelaId, String year) throws SQLException{
		...
		try{
			ps = con.prepareStatement("SELECT * " +
					" FROM CAT_SEGURO WHERE ESCUELA_ID = TO_NUMBER(?, '999')" +
	...
	public boolean existeReg(Connection conn) throws SQLException{
		...
		try{
			ps = conn.prepareStatement("SELECT * FROM CAT_SEGURO WHERE ESCUEAL_ID = TO_NUMBER(?, '999') AND YEAR = TO_NUMBER(?, '999' ");
			ps.setString(1, escuelaId);
			ps.setString(2, year);
